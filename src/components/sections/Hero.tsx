...
import NeuralAIVisualization from "@/components/ui/NeuralAIVisualization";
import { TRUSTED_BY } from "@/lib/constants";
import { fadeInUp, staggerContainer, letterStagger, letterChild, slideInRight } from "@/lib/motion";

const HEADLINE_WORDS = ["Stop", "Renting", "Your", "Time."];
      ...
      >
        <div className="flex flex-col lg:flex-row lg:items-center lg:gap-8 xl:gap-12">
          ...
          <motion.div
            ...
            initial="hidden"
            animate="visible"
            className="max-w-5xl lg:max-w-none lg:flex-1"
          >
            {/* Kicker */}
            ...
            {/* Headline - word-grouped to prevent mid-word breaks */}
            <motion.h1
              variants={letterStagger}
              className="font-serif text-5xl sm:text-6xl md:text-7xl lg:text-7xl xl:text-8xl font-bold leading-[0.95] tracking-tight mb-4"
            >
            ...
            <motion.h2
              variants={fadeInUp}
              custom={2}
              className="font-serif text-3xl sm:text-4xl md:text-5xl lg:text-5xl xl:text-6xl font-bold leading-[0.95] tracking-tight text-gradient mb-8"
            >
              {SUBHEADLINE}
            </motion.h2>
          ...
          {/* Right column - 3D Neural AI Visualization */}
          <motion.div
            variants={slideInRight}
            initial="hidden"
            animate="visible"
            className="hidden lg:flex lg:flex-1 lg:max-w-[520px] xl:max-w-[580px] items-center justify-center"
          >
            <NeuralAIVisualization />
          </motion.div>
        </div>
      </motion.div>
